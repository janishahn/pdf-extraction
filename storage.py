...
def create_initial_state(pdf_path: str, page_count: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "pages": {
            str(i + 1): {"approved": False, "workflow": {"stage": 1}, "masks": []}
...
def migrate_old_state_format(old_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"page_count": old_state["page_count"], "pages": new_pages}
...
def save_state(pdf_path: str, state: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", encoding="utf-8", dir=json_dir, delete=False, suffix=".tmp"
        ) as temp_file:
...
def create_mask(
    ...
    mask_type: str = "image",
    associated_image_ids: Optional[List[str]] = None,
