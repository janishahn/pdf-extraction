    ...
    try:
        ...
        for drawing in drawings:
            ...
            if "rect" in drawing:
                ...
                is_target_graphic = (
                    abs(box_width - TARGET_GRAPHIC_WIDTH) < DIMENSION_TOLERANCE
                    and abs(box_height - TARGET_GRAPHIC_HEIGHT) < DIMENSION_TOLERANCE
                )
            ...
            if "items" in drawing:
                for item in drawing["items"]:
                    ...
                    if item_bbox:
                        ...
                        is_target_graphic = (
                            abs(box_width - TARGET_GRAPHIC_WIDTH) < DIMENSION_TOLERANCE
                            and abs(box_height - TARGET_GRAPHIC_HEIGHT)
                            < DIMENSION_TOLERANCE
                        )
                        if not is_target_graphic:
                            raw_boxes.append(scaled_box)
        ...
        for box in filtered_boxes:
            ...
            is_target_graphic_pre_padding = (
                abs(current_box_width - effective_target_width) < DIMENSION_TOLERANCE
                and abs(current_box_height - effective_target_height)
                < DIMENSION_TOLERANCE
            )
...
def _boxes_should_merge(box1: List[float], box2: List[float], tolerance: float) -> bool:
    ...
    return not (
        expanded1[2] < expanded2[0]
        or expanded2[2] < expanded1[0]
        or expanded1[3] < expanded2[1]
        or expanded2[3] < expanded1[1]
    )
